...
async fn run_test (arr : Vec < f64 > , status : Status) -> Result < String , crate :: AppError > { let mut sum = 0f64 ; for item in arr { let mut val = todo ! () ; } if status == Status :: Active { return Ok (String :: from ("Active")) ; } return Ok (String :: from ("Done")) ; }
